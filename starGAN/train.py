...
def train(processed_dir: str, test_wav_dir: str):
    ...
    all_speaker = get_speakers() # list : [TM1, TM2, SF1, SF2]
    label_enc = LabelEncoder() # https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.LabelEncoder.html
    ...
    files, names = get_files_labels(os.path.join(processed_dir, '*.npy')) # eg. file = SF1-100001_512.npy & name = SF1-100001_512
    assert len(files) > 0

    normlizer = Normalizer() # https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.Normalizer.html
    ...
    """
    exclude_dict will look like...
    {SF1:[TM1**.wav,TM2**.wav,SF2**.wav ... ],
     SF2:[TM1**.WAV,TM2**.WAV,SF1**.wav ... ],
     TM1:[...],
     TM2:[...]}
    """
